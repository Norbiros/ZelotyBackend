...
export default class Discord {
    ...
    async init(token: string) {
        ...
        try {
            await rest.put(Routes.applicationCommands('1133333135149498441'), {
                body: commandsList,
            })
        ...
        this.client.on('ready', () => {
            ...
            this.client.user?.setActivity(`Zelotygia`, {
    ...
    private async sendCalendar() {
        ...
        if (now.getHours() === 0) {
            await this.client.guilds.cache
                .get('938422856805646346')
                ?.channels.fetch('1134029781281800253')
            const channel = this.client.channels.cache.get(
                '1134029781281800253'
            )
            if (channel && channel.isTextBased()) {
                channel.send({
                    embeds: [await new CalendarCommand().execute()],
                })
            }
        }
    }
